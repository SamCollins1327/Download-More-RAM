...
__int64 WinPmem::copy_memory(unsigned __int64 start, unsigned __int64 end) {
        ...
        // REMOVE THIS SILLY SAFETY CHECK
        /*
...
__int64 WinPmem::set_aliased_access()
{
    aliased_access = 1; 
    Log(TEXT("Aliased access dump enabled.\n"));
    return 1;
}
...
__int64 WinPmem::set_acquisition_mode(unsigned __int32 mode)
{
        ...
        if (! ((mode == PMEM_MODE_PHYSICAL) || (mode == PMEM_MODE_PTE) || (mode == PMEM_MODE_IOSPACE)) )
        {
...
__int64 WinPmem::write_raw_image()
{
        ...
        __int64 current = 0;
        uint64_t fixedStart = 0x0;
        uint64_t fixedEnd = max_physical_memory_;

        if(aliased_access){
            fixedStart = max_physical_memory_;
            //fixedEnd = max_physical_memory_ + 0x1E80000 + 0x100000;
            fixedEnd   = max_physical_memory_ + 0x240000000;
        }

        result = (BOOL) copy_memory(fixedStart, fixedEnd);
                if (!result)
                {
...
WinPmem::WinPmem():
        ...
        mode_(PMEM_MODE_IOSPACE),
        default_mode_(PMEM_MODE_IOSPACE),
        metadata_(NULL),
        metadata_len_(0),
